    ...
    <>
      ...
      <div className="flex-1 overflow-auto">
        <div className="flex gap-0 px-6 border-b border-surface-variant">
          {tabs.map(({ label, filter }) => (
            <button
              ...
              className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === filter
        ...
        <div className="px-6 pt-4">
          {filtered.length === 0 ? (
            <p className="text-sm text-surface-on-variant py-8 text-center">{t.activity.noLogs}</p>
          ) : (
