      ...
      <div className="flex-1 overflow-auto p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Card className="flex items-center gap-4 py-3 px-4">
            ...
            <div>
              <p className="text-[11px] text-surface-on-variant mb-0.5">{t.dashboard.totalTasks}</p>
              <p className="text-xl font-bold text-surface-on tabular-nums">{stats.total}</p>
            </div>
          </Card>
          ...
          <Card className="flex items-center gap-4 py-3 px-4">
            ...
            <div>
              <p className="text-[11px] text-surface-on-variant mb-0.5">{t.dashboard.successTasks}</p>
              <div className="flex items-baseline gap-1.5">
                <p className="text-xl font-bold text-surface-on tabular-nums">{stats.success}</p>
                {stats.total > 0 && (
                  <span className="text-xs text-emerald-500 font-medium">{successRate}%</span>
                )}
              </div>
            </div>
          </Card>
          ...
          <Card className="border-amber-500/20 bg-amber-500/5 overflow-hidden">
            ...
            <div className="space-y-2">
              {pendingApprovals.map(({ task, resolve }) => (
                <div key={task.task_id} className="flex items-center justify-between p-3 rounded-lg bg-surface border border-white/8">
                  <div>
                    <p className="text-sm font-medium text-surface-on capitalize">{task.type} {t.dashboard.instruction}</p>
                    <p className="text-[11px] text-surface-on-variant font-mono">#{task.task_id}</p>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <Card className="lg:col-span-2 flex flex-col" style={{ height: 420 }}>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[11px] text-surface-on-variant">{t.dashboard.realtimeStream}</span>
            </div>
                  ...
                  return (
                    <div key={type}>
                      <div className="flex justify-between items-center mb-1">
                        <span className="text-xs font-medium text-surface-on capitalize">{type}</span>
                        <span className="text-[10px] text-surface-on-variant font-mono">{Math.round(pct)}%</span>
                      </div>
    ...
    <div className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-surface-variant/40 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <span className="text-xs font-medium text-surface-on truncate">{log.title}</span>
          {log.task_type && (
            <span
        ...
        <p className="text-[11px] text-surface-on-variant truncate">{log.description}</p>
      </div>
      <div className="text-right shrink-0">
        ...
        {log.duration_ms != null && log.duration_ms > 0 && (
          <p className="text-[9px] text-emerald-500/70 font-mono">{log.duration_ms}ms</p>
        )}
      </div>
    </div>
  )
