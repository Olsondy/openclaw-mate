...
import { useTasksStore, useConfigStore } from '../store'
import type { TaskType, ActivityLog } from '../types'
...
export function useTaskHandler() {
  ...
  // 收到新任务
  useTauriEvent<WsTaskPayload>('ws:task', useCallback((task) => {
    const taskType = task.type as TaskType
    const needsApproval = approvalRules[taskType] === 'always' ||
      (approvalRules[taskType] === 'sensitive_only' && task.require_approval)
  ...
  // 任务执行完成
  useTauriEvent<WsTaskResult>('ws:task_result', useCallback((result) => {
    updateLogStatus(
      result.task_id,
  ...
  // Gateway 推送事件
  useTauriEvent<{ event: string; payload: unknown }>('ws:gateway_event', useCallback((data) => {
    const log: ActivityLog = {
